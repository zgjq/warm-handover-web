...
function TimelineContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      DataService.getChecklist(Number(id)),
    ]).then(([data, saved]) => {
  ...
  const toggleItem = async (itemId: string) => {
    ...
    if (id) {
      ...
      updated.forEach(i => { map[i.id] = i.checked; });
      await DataService.setChecklist(Number(id), map);
    }
  };
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div className="text-center flex-1">
            <h1 className="text-3xl font-bold text-gray-800 dark:text-stone-100 mb-2">📅 交接时间线</h1>
            <p className="text-gray-500 dark:text-stone-400">{handover.personName} → {handover.successorName || '待确定'}</p>
          </div>
          <button
            onClick={toggle}
        ...
        <div className="bg-white dark:bg-stone-900 rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-700 dark:text-stone-300">交接进度</span>
            <span className="text-sm font-bold text-orange-600 dark:text-orange-400">{progress}%</span>
          </div>
          <div className="w-full h-3 bg-gray-200 dark:bg-stone-700 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-orange-400 to-amber-400 rounded-full transition-all duration-500"
              style={{ width: `${progress}%` }}
            />
          </div>
          <div className="mt-2 text-xs text-gray-400 dark:text-stone-500">
        ...
        <div className="space-y-6">
          {phases.map(phase => {
            ...
            return (
              <div key={phase} className={`bg-white dark:bg-stone-900 rounded-2xl shadow-lg overflow-hidden transition-all ${allDone ? 'ring-2 ring-green-200 dark:ring-green-800' : ''}`}>
                <div className={`px-6 py-4 ${allDone ? 'bg-green-50 dark:bg-green-900/20' : 'bg-gray-50 dark:bg-stone-800'} border-b border-gray-100 dark:border-stone-700`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-xs text-gray-500 dark:text-stone-400">{done}/{items.length} 完成</span>
                      </div>
                    </div>
                  ...
                  {items.map(item => (
                    <label
                      ...
                      className={`flex items-center gap-3 px-6 py-4 cursor-pointer transition-colors ${
                        item.checked ? 'bg-green-50/50 dark:bg-green-900/10' : 'hover:bg-gray-50 dark:hover:bg-stone-800/50'
                      }`}
                    >
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className={`text-sm ${item.checked ? 'text-green-700 dark:text-green-400 line-through' : 'text-gray-700 dark:text-stone-300'}`}>
                        {item.label}
                      </span>
                    </label>
                  ))}
